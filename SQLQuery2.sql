...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID (N'[SELECT_NAME_FROM_GROUP].BI_Dimension_Vuelta') AND TYPE ='U')
	DROP TABLE [SELECT_NAME_FROM_GROUP].BI_Dimension_Vuelta
GO
...
CREATE TABLE [SELECT_NAME_FROM_GROUP].BI_Dimension_Vuelta (
	VUELTA_ID INT PRIMARY KEY IDENTITY(1,1),
	NUMERO INT
);
...
ALTER TABLE [SELECT_NAME_FROM_GROUP].BI_Hechos_Medicion ADD FOREIGN KEY (VUELTA_ID) REFERENCES [SELECT_NAME_FROM_GROUP].BI_Dimension_Vuelta (VUELTA_ID);
...
IF EXISTS (SELECT NAME FROM sys.procedures WHERE object_id = OBJECT_ID ('[SELECT_NAME_FROM_GROUP].Migracion_Dimension_Vuelta') AND TYPE ='P')
	DROP PROCEDURE [SELECT_NAME_FROM_GROUP].Migracion_Dimension_Vuelta
GO
...
CREATE PROCEDURE [SELECT_NAME_FROM_GROUP].Migracion_Dimension_Vuelta AS
BEGIN
	INSERT INTO [SELECT_NAME_FROM_GROUP].BI_Dimension_Vuelta (NUMERO)
	SELECT DISTINCT TELE_AUTO_NUMERO_VUELTA
	FROM [SELECT_NAME_FROM_GROUP].Medicion
END
GO
...
BEGIN
	...
	FROM [SELECT_NAME_FROM_GROUP].Sector AS S
	JOIN (SELECT SECTOR_TIPO_ID, SECTO_TIPO FROM [SELECT_NAME_FROM_GROUP].BI_Dimension_Tipo_Sector) AS T ON T.SECTO_TIPO = S.SECTOR_TIPO
END
GO
...
BEGIN
	INSERT INTO [SELECT_NAME_FROM_GROUP].BI_Hechos_Medicion (AUTO_ID, SECTOR_ID, VUELTA_ID, TIEMPO_ID, TELE_AUTO_VELOCIDAD, CONSUMO_COMBUSTIBLE_PROMEDIO, MIN_TIEMPO_VUELTA, DESGASTE_CAJA_PROMEDIO, MOTOR_DESGASTE, DIF_PROFUNDIDAD_1, DIF_PROFUNDIDAD_2, DIF_PROFUNDIDAD_3, DIF_PROFUNDIDAD_4, DIF_FRENO_1, DIF_FRENO_2, DIF_FRENO_3, DIF_FRENO_4)
	SELECT M.ID_AUTO, DS.SECTOR_ID, V.VUELTA_ID, T.TIEMPO_ID, MAX(M.TELE_AUTO_VELOCIDAD), AVG(M.TELE_AUTO_COMBUSTIBLE), MAX(M.TELE_AUTO_TIEMPO_VUELTA), AVG(TCC.TELE_CAJA_DESGASTE), MAX(TM.TELE_MOTOR_POTENCIA) - MIN(TM.TELE_MOTOR_POTENCIA), 
	...
	MAX(CASE WHEN TF.TELE_FRENO_POSICION_ID = 1 THEN TF.TELE_FRENO_GROSOR_PASTILLA END) - MIN(CASE WHEN TF.TELE_FRENO_POSICION_ID = 1 THEN TF.TELE_FRENO_GROSOR_PASTILLA END),  
	MAX(CASE WHEN TF.TELE_FRENO_POSICION_ID = 2 THEN TF.TELE_FRENO_GROSOR_PASTILLA END) - MIN(CASE WHEN TF.TELE_FRENO_POSICION_ID = 2 THEN TF.TELE_FRENO_GROSOR_PASTILLA END), 
	MAX(CASE WHEN TF.TELE_FRENO_POSICION_ID = 3 THEN TF.TELE_FRENO_GROSOR_PASTILLA END) - MIN(CASE WHEN TF.TELE_FRENO_POSICION_ID = 3 THEN TF.TELE_FRENO_GROSOR_PASTILLA END),  
	MAX(CASE WHEN TF.TELE_FRENO_POSICION_ID = 4 THEN TF.TELE_FRENO_GROSOR_PASTILLA END) - MIN(CASE WHEN TF.TELE_FRENO_POSICION_ID = 4 THEN TF.TELE_FRENO_GROSOR_PASTILLA END) 
	...
	JOIN (SELECT TELE_AUTO_CODIGO, TELE_FRENO_POSICION_ID, TELE_FRENO_GROSOR_PASTILLA FROM [SELECT_NAME_FROM_GROUP].Telemetria_Freno) AS TF ON TF.TELE_AUTO_CODIGO = M.TELE_AUTO_CODIGO
	JOIN (SELECT VUELTA_ID, NUMERO FROM [SELECT_NAME_FROM_GROUP].BI_Dimension_Vuelta) AS V ON V.NUMERO=M.TELE_AUTO_NUMERO_VUELTA
	WHERE M.TELE_AUTO_TIEMPO_VUELTA>0
	GROUP BY M.ID_AUTO, DS.SECTOR_ID, V.VUELTA_ID, T.TIEMPO_ID
END
GO
...
BEGIN TRANSACTION
BEGIN TRY
	EXECUTE [SELECT_NAME_FROM_GROUP].Migracion_Dimension_Vuelta
...
-- SELECT * FROM [SELECT_NAME_FROM_GROUP].BI_Dimension_Vuelta
...
CREATE VIEW [SELECT_NAME_FROM_GROUP].Vista_Desgaste AS
	SELECT A.AUTO_ID, V.NUMERO, S.CIRCUITO_CODIGO, AVG(M.DESGASTE_CAJA_PROMEDIO) AS 'DESGASTE CAJA CAMBIOS', AVG(M.MOTOR_DESGASTE) AS 'DESGASTE MOTOR', 
	...
	JOIN [SELECT_NAME_FROM_GROUP].BI_Dimension_Sector AS S ON S.SECTOR_ID=M.SECTOR_ID
	JOIN [SELECT_NAME_FROM_GROUP].BI_Dimension_Auto AS A ON A.AUTO_ID=M.AUTO_ID
	JOIN [SELECT_NAME_FROM_GROUP].BI_Dimension_Vuelta AS V ON V.VUELTA_ID=M.VUELTA_ID
	GROUP BY A.AUTO_ID, V.NUMERO, S.CIRCUITO_CODIGO
GO
