...
ALTER TABLE [SELECT_NAME_FROM_GROUP].Hechos_Medicion ADD FOREIGN KEY (SECTOR_ID) REFERENCES [SELECT_NAME_FROM_GROUP].Dimension_Sector (SECTOR_ID);
...
BEGIN
	INSERT INTO [SELECT_NAME_FROM_GROUP].Dimension_Tiempo (ANIO, CUATRIMESTRE)
...
BEGIN
	...
	JOIN (SELECT TIEMPO_ID, ANIO, CUATRIMESTRE FROM [SELECT_NAME_FROM_GROUP].Dimension_Tiempo) AS M ON M.ANIO = YEAR(N.CARRERA_FECHA) AND M.CUATRIMESTRE = (CASE
...
BEGIN
	...
	JOIN (SELECT TIEMPO_ID, ANIO, CUATRIMESTRE FROM [SELECT_NAME_FROM_GROUP].Dimension_Tiempo) AS T ON T.ANIO = YEAR(C.CARRERA_FECHA) AND T.CUATRIMESTRE = (CASE
																																								WHEN MONTH(C.CARRERA_FECHA) IN (1,2,3,4) THEN 1
																																								WHEN MONTH(C.CARRERA_FECHA) IN (5,6,7,8) THEN 2
																																								ELSE 3
																																						    END)
...
BEGIN
	INSERT INTO [SELECT_NAME_FROM_GROUP].Hechos_Medicion (AUTO_ID, SECTOR_ID, TELE_AUTO_NUMERO_VUELTA, TIEMPO_ID, TELE_AUTO_VELOCIDAD, CONSUMO_COMBUSTIBLE_PROMEDIO, MIN_TIEMPO_VUELTA, DESGASTE_CAJA_PROMEDIO, MOTOR_DESGASTE, DIF_PROFUNDIDAD_1, DIF_PROFUNDIDAD_2, DIF_PROFUNDIDAD_3, DIF_PROFUNDIDAD_4, DIF_FRENO_1, DIF_FRENO_2, DIF_FRENO_3, DIF_FRENO_4)
	SELECT M.ID_AUTO, DS.SECTOR_ID, M.TELE_AUTO_NUMERO_VUELTA, T.TIEMPO_ID, MAX(M.TELE_AUTO_VELOCIDAD), AVG(M.TELE_AUTO_COMBUSTIBLE), MAX(M.TELE_AUTO_TIEMPO_VUELTA), AVG(TCC.TELE_CAJA_DESGASTE), MAX(TM.TELE_MOTOR_POTENCIA) - MIN(TM.TELE_MOTOR_POTENCIA), NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL
	FROM [SELECT_NAME_FROM_GROUP].Medicion AS M
	JOIN (SELECT SECTOR_ID, SECTO_TIPO_ID, CIRCUITO_CODIGO FROM [SELECT_NAME_FROM_GROUP].Dimension_Sector) AS DS ON M.CODIGO_SECTOR = DS.SECTOR_ID
	...
	-- JOIN (SELECT TELE_AUTO_CODIGO, TELE_NEUMATICO_PROFUNDIDAD FROM SELECT_NAME_FROM_GROUP.Telemetria_Neumatico) AS TN 	
...
CREATE VIEW [SELECT_NAME_FROM_GROUP].Vista_Cantidad_Paradas AS
	...
	FROM [SELECT_NAME_FROM_GROUP].Hechos_Parada_Box AS H
	JOIN (SELECT TIEMPO_ID, ANIO FROM SELECT_NAME_FROM_GROUP.Dimension_Tiempo) AS T ON H.TIEMPO_ID = T.TIEMPO_ID
...
CREATE VIEW [SELECT_NAME_FROM_GROUP].Vista_Max_Velocidad AS
	...
	JOIN [SELECT_NAME_FROM_GROUP].Dimension_Sector AS S ON S.SECTOR_ID=M.SECTOR_ID
	JOIN [SELECT_NAME_FROM_GROUP].Dimension_Tipo_Sector AS TS ON TS.SECTOR_TIPO_ID=S.SECTO_TIPO_ID
...
CREATE VIEW [SELECT_NAME_FROM_GROUP].Vista_Promedio_Incidentes AS
	SELECT I.ESCUDERIA_ID, T.ANIO, 
		((SELECT COUNT(INCIDENTE_ID) FROM [SELECT_NAME_FROM_GROUP].Hechos_Incidente AS I2 
			...
			JOIN (SELECT TIEMPO_ID, ANIO, CUATRIMESTRE FROM [SELECT_NAME_FROM_GROUP].Dimension_Tiempo) AS T2 ON T2.TIEMPO_ID = I2.TIEMPO_ID
	...
	FROM [SELECT_NAME_FROM_GROUP].Hechos_Incidente AS I
	JOIN (SELECT TIEMPO_ID, ANIO, CUATRIMESTRE FROM [SELECT_NAME_FROM_GROUP].Dimension_Tiempo) AS T ON T.TIEMPO_ID = I.TIEMPO_ID
...
CREATE VIEW [SELECT_NAME_FROM_GROUP].Vista_Mayor_Consumo_Combustible AS
	...
	FROM [SELECT_NAME_FROM_GROUP].Hechos_Medicion AS M
	JOIN [SELECT_NAME_FROM_GROUP].Dimension_Sector AS S ON M.SECTOR_ID=S.SECTOR_ID
	GROUP BY S.CIRCUITO_CODIGO
...
CREATE VIEW [SELECT_NAME_FROM_GROUP].Vista_Mejor_Tiempo_Vuelta AS
	...
	FROM [SELECT_NAME_FROM_GROUP].Hechos_Medicion AS M
	JOIN [SELECT_NAME_FROM_GROUP].Dimension_Sector AS S ON S.SECTOR_ID=M.SECTOR_ID
	JOIN [SELECT_NAME_FROM_GROUP].Dimension_Auto AS A ON A.AUTO_ID=M.AUTO_ID
	JOIN [SELECT_NAME_FROM_GROUP].Dimension_Tiempo AS T ON T.TIEMPO_ID=M.TIEMPO_ID
