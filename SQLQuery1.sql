...
-- Medicion
ALTER TABLE [SELECT_NAME_FROM_GROUP].Medicion ADD FOREIGN KEY (ID_AUTO) REFERENCES [SELECT_NAME_FROM_GROUP].Auto_Tabla (ID_AUTO);
...
ALTER TABLE [SELECT_NAME_FROM_GROUP].Telemetria_Freno ADD FOREIGN KEY (TELE_AUTO_CODIGO) REFERENCES [SELECT_NAME_FROM_GROUP].Medicion (TELE_AUTO_CODIGO);
...
ALTER TABLE [SELECT_NAME_FROM_GROUP].Telemetria_Neumatico ADD FOREIGN KEY (TELE_AUTO_CODIGO) REFERENCES [SELECT_NAME_FROM_GROUP].Medicion (TELE_AUTO_CODIGO);
...
-- Parada_Box
ALTER TABLE [SELECT_NAME_FROM_GROUP].Parada_Box ADD FOREIGN KEY (ID_AUTO) REFERENCES [SELECT_NAME_FROM_GROUP].Auto_Tabla (ID_AUTO);
ALTER TABLE [SELECT_NAME_FROM_GROUP].Parada_Box ADD FOREIGN KEY (ID_CARRERA) REFERENCES [SELECT_NAME_FROM_GROUP].Carrera (CODIGO_CARRERA);
...
IF EXISTS (SELECT NAME FROM sys.procedures WHERE object_id = OBJECT_ID ('[SELECT_NAME_FROM_GROUP].migracion_telemetria_neumatico') AND TYPE ='P')
	DROP PROCEDURE [SELECT_NAME_FROM_GROUP].migracion_telemetria_neumatico
GO
...
BEGIN
	INSERT INTO [SELECT_NAME_FROM_GROUP].Parada_Box (PARADA_BOX_TIEMPO, PARADA_BOX_VUELTA, ID_AUTO, ID_CARRERA)
	SELECT DISTINCT M.PARADA_BOX_TIEMPO, M.PARADA_BOX_VUELTA, A.ID_AUTO, M.CODIGO_CARRERA
	...
	WHERE M.PARADA_BOX_TIEMPO IS NOT NULL AND M.PARADA_BOX_VUELTA IS NOT NULL AND M.AUTO_MODELO IS NOT NULL AND M.AUTO_NUMERO IS NOT NULL
END
GO
...
BEGIN
	...
	FROM [gd_esquema].Maestra AS M
	JOIN (SELECT CIRCUITO_CODIGO FROM [SELECT_NAME_FROM_GROUP].Circuito) AS C ON C.CIRCUITO_CODIGO = M.CIRCUITO_CODIGO
...
BEGIN
	...
	SELECT DISTINCT M.TELE_NEUMATICO1_NRO_SERIE
	, (SELECT TOP 1 P.ID_PARADA_BOX FROM [SELECT_NAME_FROM_GROUP].Parada_Box P JOIN gd_esquema.Maestra M2 ON NEUMATICO1_NRO_SERIE_NUEVO=M.TELE_NEUMATICO1_NRO_SERIE WHERE P.PARADA_BOX_TIEMPO=M2.PARADA_BOX_TIEMPO AND P.PARADA_BOX_VUELTA=M2.PARADA_BOX_VUELTA) 
	, (SELECT TOP 1 P.ID_PARADA_BOX FROM [SELECT_NAME_FROM_GROUP].Parada_Box P JOIN gd_esquema.Maestra M2 ON NEUMATICO1_NRO_SERIE_VIEJO=M.TELE_NEUMATICO1_NRO_SERIE WHERE P.PARADA_BOX_TIEMPO=M2.PARADA_BOX_TIEMPO AND P.PARADA_BOX_VUELTA=M2.PARADA_BOX_VUELTA) 
	, (SELECT TN.ID_TIPO_NEUMATICO FROM [SELECT_NAME_FROM_GROUP].Tipo_Neumatico TN WHERE TN.TIPO_DESCRIPCION=M.NEUMATICO1_TIPO_VIEJO)
	FROM gd_esquema.Maestra M
	WHERE TELE_NEUMATICO1_NRO_SERIE IS NOT NULL
		UNION
	SELECT DISTINCT M.TELE_NEUMATICO2_NRO_SERIE
	, (SELECT TOP 1 P.ID_PARADA_BOX FROM [SELECT_NAME_FROM_GROUP].Parada_Box P JOIN gd_esquema.Maestra M2 ON NEUMATICO1_NRO_SERIE_NUEVO=M.TELE_NEUMATICO2_NRO_SERIE WHERE P.PARADA_BOX_TIEMPO=M2.PARADA_BOX_TIEMPO AND P.PARADA_BOX_VUELTA=M2.PARADA_BOX_VUELTA) 
	, (SELECT TOP 1 P.ID_PARADA_BOX FROM [SELECT_NAME_FROM_GROUP].Parada_Box P JOIN gd_esquema.Maestra M2 ON NEUMATICO1_NRO_SERIE_VIEJO=M.TELE_NEUMATICO2_NRO_SERIE WHERE P.PARADA_BOX_TIEMPO=M2.PARADA_BOX_TIEMPO AND P.PARADA_BOX_VUELTA=M2.PARADA_BOX_VUELTA) 
	, (SELECT TN.ID_TIPO_NEUMATICO FROM [SELECT_NAME_FROM_GROUP].Tipo_Neumatico TN WHERE TN.TIPO_DESCRIPCION=M.NEUMATICO2_TIPO_VIEJO)
	FROM gd_esquema.Maestra M
	WHERE TELE_NEUMATICO2_NRO_SERIE IS NOT NULL
		UNION
	SELECT DISTINCT M.TELE_NEUMATICO3_NRO_SERIE
	, (SELECT TOP 1 P.ID_PARADA_BOX FROM [SELECT_NAME_FROM_GROUP].Parada_Box P JOIN gd_esquema.Maestra M2 ON NEUMATICO1_NRO_SERIE_NUEVO=M.TELE_NEUMATICO3_NRO_SERIE WHERE P.PARADA_BOX_TIEMPO=M2.PARADA_BOX_TIEMPO AND P.PARADA_BOX_VUELTA=M2.PARADA_BOX_VUELTA) 
	, (SELECT TOP 1 P.ID_PARADA_BOX FROM [SELECT_NAME_FROM_GROUP].Parada_Box P JOIN gd_esquema.Maestra M2 ON NEUMATICO1_NRO_SERIE_VIEJO=M.TELE_NEUMATICO3_NRO_SERIE WHERE P.PARADA_BOX_TIEMPO=M2.PARADA_BOX_TIEMPO AND P.PARADA_BOX_VUELTA=M2.PARADA_BOX_VUELTA) 
	, (SELECT TN.ID_TIPO_NEUMATICO FROM [SELECT_NAME_FROM_GROUP].Tipo_Neumatico TN WHERE TN.TIPO_DESCRIPCION=M.NEUMATICO3_TIPO_VIEJO)
	FROM gd_esquema.Maestra M
	WHERE TELE_NEUMATICO3_NRO_SERIE IS NOT NULL
		UNION
	SELECT DISTINCT M.TELE_NEUMATICO4_NRO_SERIE
	, (SELECT TOP 1 P.ID_PARADA_BOX FROM [SELECT_NAME_FROM_GROUP].Parada_Box P JOIN gd_esquema.Maestra M2 ON NEUMATICO1_NRO_SERIE_NUEVO=M.TELE_NEUMATICO4_NRO_SERIE WHERE P.PARADA_BOX_TIEMPO=M2.PARADA_BOX_TIEMPO AND P.PARADA_BOX_VUELTA=M2.PARADA_BOX_VUELTA) 
	, (SELECT TOP 1 P.ID_PARADA_BOX FROM [SELECT_NAME_FROM_GROUP].Parada_Box P JOIN gd_esquema.Maestra M2 ON NEUMATICO1_NRO_SERIE_VIEJO=M.TELE_NEUMATICO4_NRO_SERIE WHERE P.PARADA_BOX_TIEMPO=M2.PARADA_BOX_TIEMPO AND P.PARADA_BOX_VUELTA=M2.PARADA_BOX_VUELTA) 
	, (SELECT TN.ID_TIPO_NEUMATICO FROM [SELECT_NAME_FROM_GROUP].Tipo_Neumatico TN WHERE TN.TIPO_DESCRIPCION=M.NEUMATICO4_TIPO_VIEJO)
	FROM gd_esquema.Maestra M
...
BEGIN
	INSERT INTO [SELECT_NAME_FROM_GROUP].Telemetria_Freno (TELE_FRENO_POSICION_ID, TELE_FRENO_TEMPERATURA, TELE_FRENO_GROSOR_PASTILLA, TELE_AUTO_CODIGO, FRENO_ID)
	SELECT DISTINCT P1.ID_POSICION, TELE_FRENO1_TEMPERATURA, TELE_FRENO1_GROSOR_PASTILLA, MED1.TELE_AUTO_CODIGO, F1.FRENO_ID FROM [gd_esquema].Maestra AS M1
	JOIN (SELECT ID_POSICION, DESCRIPCION FROM [SELECT_NAME_FROM_GROUP].Posicion) AS P1 ON P1.DESCRIPCION=M1.TELE_FRENO1_POSICION
	JOIN (SELECT TELE_AUTO_CODIGO, TELE_AUTO_NUMERO_VUELTA, TELE_AUTO_DISTANCIA_CARRERA, TELE_AUTO_DISTANCIA_VUELTA, TELE_AUTO_TIEMPO_VUELTA, TELE_AUTO_POSICION, TELE_AUTO_VELOCIDAD, TELE_AUTO_COMBUSTIBLE FROM [SELECT_NAME_FROM_GROUP].Medicion) AS MED1 ON MED1.TELE_AUTO_CODIGO=M1.TELE_AUTO_CODIGO AND MED1.TELE_AUTO_NUMERO_VUELTA=M1.TELE_AUTO_NUMERO_VUELTA AND MED1.TELE_AUTO_DISTANCIA_CARRERA=M1.TELE_AUTO_DISTANCIA_CARRERA AND MED1.TELE_AUTO_DISTANCIA_VUELTA=M1.TELE_AUTO_DISTANCIA_VUELTA AND MED1.TELE_AUTO_TIEMPO_VUELTA=M1.TELE_AUTO_TIEMPO_VUELTA AND MED1.TELE_AUTO_POSICION=M1.TELE_AUTO_POSICION AND MED1.TELE_AUTO_VELOCIDAD=M1.TELE_AUTO_VELOCIDAD AND MED1.TELE_AUTO_COMBUSTIBLE=M1.TELE_AUTO_COMBUSTIBLE
	JOIN (SELECT FRENO_ID, FRENO_NRO_SERIE FROM [SELECT_NAME_FROM_GROUP].Freno) AS F1 ON F1.FRENO_NRO_SERIE=M1.TELE_FRENO1_NRO_SERIE
	WHERE TELE_FRENO1_TEMPERATURA IS NOT NULL AND TELE_FRENO1_GROSOR_PASTILLA IS NOT NULL
		UNION
	SELECT DISTINCT P2.ID_POSICION, TELE_FRENO2_TEMPERATURA, TELE_FRENO2_GROSOR_PASTILLA, MED2.TELE_AUTO_CODIGO, F2.FRENO_ID FROM [gd_esquema].Maestra AS M2
	JOIN (SELECT ID_POSICION, DESCRIPCION FROM [SELECT_NAME_FROM_GROUP].Posicion) AS P2 ON P2.DESCRIPCION=M2.TELE_FRENO2_POSICION
	JOIN (SELECT TELE_AUTO_CODIGO, TELE_AUTO_NUMERO_VUELTA, TELE_AUTO_DISTANCIA_CARRERA, TELE_AUTO_DISTANCIA_VUELTA, TELE_AUTO_TIEMPO_VUELTA, TELE_AUTO_POSICION, TELE_AUTO_VELOCIDAD, TELE_AUTO_COMBUSTIBLE FROM [SELECT_NAME_FROM_GROUP].Medicion) AS MED2 ON MED2.TELE_AUTO_CODIGO=M2.TELE_AUTO_CODIGO AND MED2.TELE_AUTO_NUMERO_VUELTA=M2.TELE_AUTO_NUMERO_VUELTA AND MED2.TELE_AUTO_DISTANCIA_CARRERA=M2.TELE_AUTO_DISTANCIA_CARRERA AND MED2.TELE_AUTO_DISTANCIA_VUELTA=M2.TELE_AUTO_DISTANCIA_VUELTA AND MED2.TELE_AUTO_TIEMPO_VUELTA=M2.TELE_AUTO_TIEMPO_VUELTA AND MED2.TELE_AUTO_POSICION=M2.TELE_AUTO_POSICION AND MED2.TELE_AUTO_VELOCIDAD=M2.TELE_AUTO_VELOCIDAD AND MED2.TELE_AUTO_COMBUSTIBLE=M2.TELE_AUTO_COMBUSTIBLE
	JOIN (SELECT FRENO_ID, FRENO_NRO_SERIE FROM [SELECT_NAME_FROM_GROUP].Freno) AS F2 ON F2.FRENO_NRO_SERIE=M2.TELE_FRENO2_NRO_SERIE
	WHERE TELE_FRENO2_TEMPERATURA IS NOT NULL AND TELE_FRENO2_GROSOR_PASTILLA IS NOT NULL
		UNION
	SELECT DISTINCT P3.ID_POSICION, TELE_FRENO3_TEMPERATURA, TELE_FRENO3_GROSOR_PASTILLA, MED3.TELE_AUTO_CODIGO, F3.FRENO_ID FROM [gd_esquema].Maestra AS M3
	JOIN (SELECT ID_POSICION, DESCRIPCION FROM [SELECT_NAME_FROM_GROUP].Posicion) AS P3 ON P3.DESCRIPCION=M3.TELE_FRENO3_POSICION
	JOIN (SELECT TELE_AUTO_CODIGO, TELE_AUTO_NUMERO_VUELTA, TELE_AUTO_DISTANCIA_CARRERA, TELE_AUTO_DISTANCIA_VUELTA, TELE_AUTO_TIEMPO_VUELTA, TELE_AUTO_POSICION, TELE_AUTO_VELOCIDAD, TELE_AUTO_COMBUSTIBLE FROM [SELECT_NAME_FROM_GROUP].Medicion) AS MED3 ON MED3.TELE_AUTO_CODIGO=M3.TELE_AUTO_CODIGO AND MED3.TELE_AUTO_NUMERO_VUELTA=M3.TELE_AUTO_NUMERO_VUELTA AND MED3.TELE_AUTO_DISTANCIA_CARRERA=M3.TELE_AUTO_DISTANCIA_CARRERA AND MED3.TELE_AUTO_DISTANCIA_VUELTA=M3.TELE_AUTO_DISTANCIA_VUELTA AND MED3.TELE_AUTO_TIEMPO_VUELTA=M3.TELE_AUTO_TIEMPO_VUELTA AND MED3.TELE_AUTO_POSICION=M3.TELE_AUTO_POSICION AND MED3.TELE_AUTO_VELOCIDAD=M3.TELE_AUTO_VELOCIDAD AND MED3.TELE_AUTO_COMBUSTIBLE=M3.TELE_AUTO_COMBUSTIBLE
	JOIN (SELECT FRENO_ID, FRENO_NRO_SERIE FROM [SELECT_NAME_FROM_GROUP].Freno) AS F3 ON F3.FRENO_NRO_SERIE=M3.TELE_FRENO3_NRO_SERIE
	WHERE TELE_FRENO3_TEMPERATURA IS NOT NULL AND TELE_FRENO3_GROSOR_PASTILLA IS NOT NULL
		UNION
	SELECT DISTINCT P4.ID_POSICION, TELE_FRENO4_TEMPERATURA, TELE_FRENO4_GROSOR_PASTILLA, MED4.TELE_AUTO_CODIGO, F4.FRENO_ID FROM [gd_esquema].Maestra AS M4
	JOIN (SELECT ID_POSICION, DESCRIPCION FROM [SELECT_NAME_FROM_GROUP].Posicion) AS P4 ON P4.DESCRIPCION=M4.TELE_FRENO4_POSICION
	JOIN (SELECT TELE_AUTO_CODIGO, TELE_AUTO_NUMERO_VUELTA, TELE_AUTO_DISTANCIA_CARRERA, TELE_AUTO_DISTANCIA_VUELTA, TELE_AUTO_TIEMPO_VUELTA, TELE_AUTO_POSICION, TELE_AUTO_VELOCIDAD, TELE_AUTO_COMBUSTIBLE FROM [SELECT_NAME_FROM_GROUP].Medicion) AS MED4 ON MED4.TELE_AUTO_CODIGO=M4.TELE_AUTO_CODIGO AND MED4.TELE_AUTO_NUMERO_VUELTA=M4.TELE_AUTO_NUMERO_VUELTA AND MED4.TELE_AUTO_DISTANCIA_CARRERA=M4.TELE_AUTO_DISTANCIA_CARRERA AND MED4.TELE_AUTO_DISTANCIA_VUELTA=M4.TELE_AUTO_DISTANCIA_VUELTA AND MED4.TELE_AUTO_TIEMPO_VUELTA=M4.TELE_AUTO_TIEMPO_VUELTA AND MED4.TELE_AUTO_POSICION=M4.TELE_AUTO_POSICION AND MED4.TELE_AUTO_VELOCIDAD=M4.TELE_AUTO_VELOCIDAD AND MED4.TELE_AUTO_COMBUSTIBLE=M4.TELE_AUTO_COMBUSTIBLE
	JOIN (SELECT FRENO_ID, FRENO_NRO_SERIE FROM [SELECT_NAME_FROM_GROUP].Freno) AS F4 ON F4.FRENO_NRO_SERIE=M4.TELE_FRENO4_NRO_SERIE
...
BEGIN
	...
	FROM [gd_esquema].Maestra AS M
	JOIN (SELECT MOTOR_ID, MOTOR_NRO_SERIE FROM [SELECT_NAME_FROM_GROUP].Motor) AS MOT ON MOT.MOTOR_NRO_SERIE=M.TELE_MOTOR_NRO_SERIE
...
BEGIN
	...
	FROM [gd_esquema].Maestra AS M
	JOIN (SELECT CAJA_CAMBIO_ID, CAJA_NRO_SERIE FROM [SELECT_NAME_FROM_GROUP].Caja_Cambios) AS C ON C.CAJA_NRO_SERIE=M.TELE_CAJA_NRO_SERIE
...
BEGIN
	INSERT INTO [SELECT_NAME_FROM_GROUP].Incidente (CODIGO_BANDERA, INCIDENTE_TIEMPO, CODIGO_SECTOR, CARRERA_FECHA)
	SELECT DISTINCT B.ID_BANDERA, M.INCIDENTE_TIEMPO, S.CODIGO_SECTOR, M.CARRERA_FECHA
	FROM [gd_esquema].Maestra AS M
	...
	JOIN (SELECT CODIGO_SECTOR FROM [SELECT_NAME_FROM_GROUP].Sector) AS S ON S.CODIGO_SECTOR = M.CODIGO_SECTOR
	WHERE ID_BANDERA IS NOT NULL AND M.INCIDENTE_TIEMPO IS NOT NULL AND S.CODIGO_SECTOR IS NOT NULL AND M.CARRERA_FECHA IS NOT NULL
END
GO
...
BEGIN
	...
	FROM [gd_esquema].Maestra AS M
	JOIN (SELECT ID_AUTO, AUTO_MODELO, AUTO_NUMERO FROM [SELECT_NAME_FROM_GROUP].Auto_Tabla) AS A ON A.AUTO_MODELO=M.AUTO_MODELO AND A.AUTO_NUMERO=M.AUTO_NUMERO
	JOIN (SELECT ID_INCIDENTE, INCIDENTE_TIEMPO, CODIGO_SECTOR FROM [SELECT_NAME_FROM_GROUP].Incidente) AS I ON I.INCIDENTE_TIEMPO = M.INCIDENTE_TIEMPO AND I.CODIGO_SECTOR = M.CODIGO_SECTOR
...
BEGIN
	INSERT INTO [SELECT_NAME_FROM_GROUP].Medicion (TELE_AUTO_CODIGO, ID_AUTO, TELE_AUTO_NUMERO_VUELTA, TELE_AUTO_DISTANCIA_CARRERA, TELE_AUTO_DISTANCIA_VUELTA, TELE_AUTO_TIEMPO_VUELTA, TELE_AUTO_POSICION, TELE_AUTO_VELOCIDAD, TELE_AUTO_COMBUSTIBLE, CODIGO_CARRERA, CODIGO_SECTOR, TELE_MOTOR_ID, TELE_CAJA_ID)
	SELECT DISTINCT FLOOR(M.TELE_AUTO_CODIGO), A.ID_AUTO, M.TELE_AUTO_NUMERO_VUELTA, M.TELE_AUTO_DISTANCIA_CARRERA, M.TELE_AUTO_DISTANCIA_VUELTA, M.TELE_AUTO_TIEMPO_VUELTA, M.TELE_AUTO_POSICION, M.TELE_AUTO_VELOCIDAD, M.TELE_AUTO_COMBUSTIBLE, M.CODIGO_CARRERA, M.CODIGO_SECTOR
	...
	FROM [gd_esquema].Maestra AS M
	JOIN (SELECT ID_AUTO, AUTO_MODELO, AUTO_NUMERO FROM [SELECT_NAME_FROM_GROUP].Auto_Tabla) AS A ON M.AUTO_MODELO=A.AUTO_MODELO AND M.AUTO_NUMERO=A.AUTO_NUMERO
	WHERE	M.TELE_AUTO_CODIGO IS NOT NULL AND
			M.TELE_AUTO_NUMERO_VUELTA IS NOT NULL AND 
			M.TELE_AUTO_DISTANCIA_CARRERA IS NOT NULL AND 
			M.TELE_AUTO_DISTANCIA_VUELTA IS NOT NULL AND 
			M.TELE_AUTO_TIEMPO_VUELTA IS NOT NULL AND 
			M.TELE_AUTO_POSICION IS NOT NULL AND 
			M.TELE_AUTO_VELOCIDAD IS NOT NULL AND 
			M.TELE_AUTO_COMBUSTIBLE IS NOT NULL AND
			M.TELE_MOTOR_MODELO IS NOT NULL AND
			M.TELE_CAJA_MODELO IS NOT NULL
END
GO

CREATE PROCEDURE [SELECT_NAME_FROM_GROUP].migracion_telemetria_neumatico AS
BEGIN
	INSERT INTO [SELECT_NAME_FROM_GROUP].Telemetria_Neumatico (TELE_NEUMATICO_PRESION, TELE_NEUMATICO_PROFUNDIDAD, TELE_NEUMATICO_TEMPERATURA, TELE_AUTO_CODIGO, NEUMATICO_ID, TELE_NEUMATICO_POSICION_ID)
	SELECT DISTINCT M.TELE_NEUMATICO1_PRESION, M.TELE_NEUMATICO1_PROFUNDIDAD, M.TELE_NEUMATICO1_TEMPERATURA, MED.TELE_AUTO_CODIGO, N.ID_NEUMATICO, P.ID_POSICION
	FROM [gd_esquema].Maestra AS M
	JOIN (SELECT TELE_AUTO_CODIGO, TELE_AUTO_NUMERO_VUELTA, TELE_AUTO_DISTANCIA_CARRERA, TELE_AUTO_DISTANCIA_VUELTA, TELE_AUTO_TIEMPO_VUELTA, TELE_AUTO_POSICION, TELE_AUTO_VELOCIDAD, TELE_AUTO_COMBUSTIBLE FROM [SELECT_NAME_FROM_GROUP].Medicion) AS MED ON MED.TELE_AUTO_CODIGO=M.TELE_AUTO_CODIGO AND MED.TELE_AUTO_NUMERO_VUELTA=M.TELE_AUTO_NUMERO_VUELTA AND MED.TELE_AUTO_DISTANCIA_CARRERA=M.TELE_AUTO_DISTANCIA_CARRERA AND MED.TELE_AUTO_DISTANCIA_VUELTA=M.TELE_AUTO_DISTANCIA_VUELTA AND MED.TELE_AUTO_TIEMPO_VUELTA=M.TELE_AUTO_TIEMPO_VUELTA AND MED.TELE_AUTO_POSICION=M.TELE_AUTO_POSICION AND MED.TELE_AUTO_VELOCIDAD=M.TELE_AUTO_VELOCIDAD AND MED.TELE_AUTO_COMBUSTIBLE=M.TELE_AUTO_COMBUSTIBLE
	JOIN (SELECT ID_NEUMATICO, NEUMATICO_NRO_SERIE FROM [SELECT_NAME_FROM_GROUP].Neumatico) AS N ON N.NEUMATICO_NRO_SERIE=M.TELE_NEUMATICO1_NRO_SERIE 
	JOIN (SELECT ID_POSICION, DESCRIPCION FROM [SELECT_NAME_FROM_GROUP].Posicion) AS P ON M.TELE_NEUMATICO1_POSICION=P.DESCRIPCION
	WHERE	M.TELE_AUTO_CODIGO IS NOT NULL AND M.TELE_NEUMATICO1_PRESION IS NOT NULL AND M.TELE_NEUMATICO1_PROFUNDIDAD IS NOT NULL AND M.TELE_NEUMATICO1_TEMPERATURA IS NOT NULL
		UNION
	SELECT DISTINCT M.TELE_NEUMATICO2_PRESION, M.TELE_NEUMATICO2_PROFUNDIDAD, M.TELE_NEUMATICO2_TEMPERATURA, MED.TELE_AUTO_CODIGO, N.ID_NEUMATICO, P.ID_POSICION
	FROM [gd_esquema].Maestra AS M
	JOIN (SELECT TELE_AUTO_CODIGO, TELE_AUTO_NUMERO_VUELTA, TELE_AUTO_DISTANCIA_CARRERA, TELE_AUTO_DISTANCIA_VUELTA, TELE_AUTO_TIEMPO_VUELTA, TELE_AUTO_POSICION, TELE_AUTO_VELOCIDAD, TELE_AUTO_COMBUSTIBLE FROM [SELECT_NAME_FROM_GROUP].Medicion) AS MED ON MED.TELE_AUTO_CODIGO=M.TELE_AUTO_CODIGO AND MED.TELE_AUTO_NUMERO_VUELTA=M.TELE_AUTO_NUMERO_VUELTA AND MED.TELE_AUTO_DISTANCIA_CARRERA=M.TELE_AUTO_DISTANCIA_CARRERA AND MED.TELE_AUTO_DISTANCIA_VUELTA=M.TELE_AUTO_DISTANCIA_VUELTA AND MED.TELE_AUTO_TIEMPO_VUELTA=M.TELE_AUTO_TIEMPO_VUELTA AND MED.TELE_AUTO_POSICION=M.TELE_AUTO_POSICION AND MED.TELE_AUTO_VELOCIDAD=M.TELE_AUTO_VELOCIDAD AND MED.TELE_AUTO_COMBUSTIBLE=M.TELE_AUTO_COMBUSTIBLE
	JOIN (SELECT ID_NEUMATICO, NEUMATICO_NRO_SERIE FROM [SELECT_NAME_FROM_GROUP].Neumatico) AS N ON N.NEUMATICO_NRO_SERIE=M.TELE_NEUMATICO2_NRO_SERIE 
	JOIN (SELECT ID_POSICION, DESCRIPCION FROM [SELECT_NAME_FROM_GROUP].Posicion) AS P ON M.TELE_NEUMATICO2_POSICION=P.DESCRIPCION
	WHERE	M.TELE_AUTO_CODIGO IS NOT NULL AND M.TELE_NEUMATICO2_PRESION IS NOT NULL AND M.TELE_NEUMATICO2_PROFUNDIDAD IS NOT NULL AND M.TELE_NEUMATICO2_TEMPERATURA IS NOT NULL
		UNION
	SELECT DISTINCT M.TELE_NEUMATICO3_PRESION, M.TELE_NEUMATICO3_PROFUNDIDAD, M.TELE_NEUMATICO3_TEMPERATURA, MED.TELE_AUTO_CODIGO, N.ID_NEUMATICO, P.ID_POSICION
	FROM [gd_esquema].Maestra AS M
	JOIN (SELECT TELE_AUTO_CODIGO, TELE_AUTO_NUMERO_VUELTA, TELE_AUTO_DISTANCIA_CARRERA, TELE_AUTO_DISTANCIA_VUELTA, TELE_AUTO_TIEMPO_VUELTA, TELE_AUTO_POSICION, TELE_AUTO_VELOCIDAD, TELE_AUTO_COMBUSTIBLE FROM [SELECT_NAME_FROM_GROUP].Medicion) AS MED ON MED.TELE_AUTO_CODIGO=M.TELE_AUTO_CODIGO AND MED.TELE_AUTO_NUMERO_VUELTA=M.TELE_AUTO_NUMERO_VUELTA AND MED.TELE_AUTO_DISTANCIA_CARRERA=M.TELE_AUTO_DISTANCIA_CARRERA AND MED.TELE_AUTO_DISTANCIA_VUELTA=M.TELE_AUTO_DISTANCIA_VUELTA AND MED.TELE_AUTO_TIEMPO_VUELTA=M.TELE_AUTO_TIEMPO_VUELTA AND MED.TELE_AUTO_POSICION=M.TELE_AUTO_POSICION AND MED.TELE_AUTO_VELOCIDAD=M.TELE_AUTO_VELOCIDAD AND MED.TELE_AUTO_COMBUSTIBLE=M.TELE_AUTO_COMBUSTIBLE
	JOIN (SELECT ID_NEUMATICO, NEUMATICO_NRO_SERIE FROM [SELECT_NAME_FROM_GROUP].Neumatico) AS N ON N.NEUMATICO_NRO_SERIE=M.TELE_NEUMATICO3_NRO_SERIE 
	JOIN (SELECT ID_POSICION, DESCRIPCION FROM [SELECT_NAME_FROM_GROUP].Posicion) AS P ON M.TELE_NEUMATICO3_POSICION=P.DESCRIPCION
	WHERE	M.TELE_AUTO_CODIGO IS NOT NULL AND M.TELE_NEUMATICO3_PRESION IS NOT NULL AND M.TELE_NEUMATICO3_PROFUNDIDAD IS NOT NULL AND M.TELE_NEUMATICO3_TEMPERATURA IS NOT NULL
		UNION
	SELECT DISTINCT M.TELE_NEUMATICO4_PRESION, M.TELE_NEUMATICO4_PROFUNDIDAD, M.TELE_NEUMATICO4_TEMPERATURA, MED.TELE_AUTO_CODIGO, N.ID_NEUMATICO, P.ID_POSICION
	FROM [gd_esquema].Maestra AS M
	JOIN (SELECT TELE_AUTO_CODIGO, TELE_AUTO_NUMERO_VUELTA, TELE_AUTO_DISTANCIA_CARRERA, TELE_AUTO_DISTANCIA_VUELTA, TELE_AUTO_TIEMPO_VUELTA, TELE_AUTO_POSICION, TELE_AUTO_VELOCIDAD, TELE_AUTO_COMBUSTIBLE FROM [SELECT_NAME_FROM_GROUP].Medicion) AS MED ON MED.TELE_AUTO_CODIGO=M.TELE_AUTO_CODIGO AND MED.TELE_AUTO_NUMERO_VUELTA=M.TELE_AUTO_NUMERO_VUELTA AND MED.TELE_AUTO_DISTANCIA_CARRERA=M.TELE_AUTO_DISTANCIA_CARRERA AND MED.TELE_AUTO_DISTANCIA_VUELTA=M.TELE_AUTO_DISTANCIA_VUELTA AND MED.TELE_AUTO_TIEMPO_VUELTA=M.TELE_AUTO_TIEMPO_VUELTA AND MED.TELE_AUTO_POSICION=M.TELE_AUTO_POSICION AND MED.TELE_AUTO_VELOCIDAD=M.TELE_AUTO_VELOCIDAD AND MED.TELE_AUTO_COMBUSTIBLE=M.TELE_AUTO_COMBUSTIBLE
	JOIN (SELECT ID_NEUMATICO, NEUMATICO_NRO_SERIE FROM [SELECT_NAME_FROM_GROUP].Neumatico) AS N ON N.NEUMATICO_NRO_SERIE=M.TELE_NEUMATICO4_NRO_SERIE 
	JOIN (SELECT ID_POSICION, DESCRIPCION FROM [SELECT_NAME_FROM_GROUP].Posicion) AS P ON M.TELE_NEUMATICO4_POSICION=P.DESCRIPCION
	WHERE	M.TELE_AUTO_CODIGO IS NOT NULL AND M.TELE_NEUMATICO4_PRESION IS NOT NULL AND M.TELE_NEUMATICO4_PROFUNDIDAD IS NOT NULL AND M.TELE_NEUMATICO4_TEMPERATURA IS NOT NULL
END
GO
...
BEGIN TRY
	...
	EXECUTE [SELECT_NAME_FROM_GROUP].migracion_telemetria_neumatico
END TRY
BEGIN CATCH
...
SELECT * FROM [SELECT_NAME_FROM_GROUP].Auto_Tabla
...
SELECT * FROM [SELECT_NAME_FROM_GROUP].Medicion --AS M JOIN SELECT_NAME_FROM_GROUP.Sector AS S ON S.CODIGO_SECTOR=M.CODIGO_SECTOR WHERE S.SECTOR_TIPO='Recta' AND ID_AUTO=3 AND TELE_AUTO_NUMERO_VUELTA=11 AND CODIGO_CARRERA=3
